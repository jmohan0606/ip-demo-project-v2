...
def attribute_group(
    ...
    # 9. VOLUME + DEAL_SIZE — transaction-based (non-recurring-class) groups.
    #
    # R8 fix. The previous step emitted ONLY the count effect
    # ((to_count - from_count) * from_avg) and left the average-value effect
    # unattributed, so it fell into MIX. On real data average transaction value
    # is not stable between months, so that missing term was large: it is the
    # single biggest contributor to the observed MIX residual.
    #
    ...
    # Emitting both makes the remaining-transaction change reconcile by
    # construction, leaving MIX to carry only genuine cross-group product mix.
    if not is_recurring_class and (rem_from or rem_to):
