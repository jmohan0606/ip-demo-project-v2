...
| 17 | 2026-07-26 | 2026-07-26 | round 16 fresh start (FIX_SPEC_R16.md) | CRITICAL key-collision fix: W-A1..W-E all DONE; version_id=v{n}\|{advisor}, scan_id=scan{n:03d}\|{advisor}, per-advisor sequences; R15.1 MaxAccum KEPT; verify_round16 43/43 (3/3 advisors survive bulk runs); all suites re-run PASS, e2e recon $0.00; tsc clean; live reinstall + data migration = operator (ROUND16_ACCEPTANCE.md) |
...
| W-A1 | R16 | commentary version_id advisor-scoped (`v{n}\|{advisor}`); version_no per-advisor sequence | DONE | a4567c4 | _latest_version_no(graph, advisor_sid): max over advisor's own + legacy global "", never a global max |
| W-A2 | R16 | update all version-id references (commentary rows, edge, evaluations, status payloads, CSV) | DONE | a4567c4 | commentary_id/evidence_id/evaluation_id/edges/payloads/CSV all BUILT FROM version_id — inherit the scoped format; frontend URL-encodes version ids (contain "\|") |
| W-A3 | R16 | get_commentary / get_commentary_versions resolve latest PER ADVISOR; supersede per advisor | DONE | a4567c4 | R15.1 MaxAccum fix KEPT (not reverted); GQ-009 header documents R16 format, no logic change (already per-advisor + vertex-id read-back); supersede switched to query-based (both tiers), advisor-scoped; NEEDS LIVE REINSTALL |
| W-B1 | R16 | anomaly scan_id advisor-scoped; scan number per-advisor | DONE | 020af98 | scan{n:03d}\|{advisor}; _next_scan_id via get_anomaly_scans (both tiers — old store-only read pinned tier 1 at scan001) |
| W-B2 | R16 | update all scan-id references (anomaly rows, edges, status payloads) | DONE | 020af98 | anomaly_id embeds scan_id; anomaly_in_scan/for_advisor/cites_driver + payloads inherit automatically; anomaly URLs already encoded |
| W-B3 | R16 | get_anomalies / get_anomaly_scans resolve latest scan PER ADVISOR | DONE | 020af98 | repo GQ-018/019 already filter advisor FIRST in both passes (R11); headers now state the contract (mirrors get_commentary) + scoped id format; NEEDS LIVE REINSTALL |
| W-C | R16 | migration steps in ROUND16_ACCEPTANCE.md (clear collided data, regenerate all); confirm no schema ALTER | DONE | 6345f7d | targeted clear (ingestion API or GSQL) + CSV header reset + regenerate-all/rescan-all; PRIMARY_ID STRING unchanged — NO ALTER, confirmed §3 |
| W-D | R16 | scripts/verify_round16.py — multi-advisor generate-all + rescan-all survival, PASS/FAIL counts | DONE | 5f7216e | 43/43 PASS: 3/3 advisors keep version+scan in bulk runs, double-run supersede per advisor, single-advisor no-clobber, zero dangling refs, per-advisor sequences |
| W-E | R16 | docs/ROUND16_CHANGED_FILES.md (git-derived, conflict flags, operator-local excluded) | DONE | dd8712b | + BUILD_REPORT §22; all suites re-run PASS (per_advisor 33/33, assistant 101/101, guardrail 54/54, round15 25/25, e2e recon $0.00, …); tsc clean; sample demo state untouched |
...
| 2026-07-26 | R16 SUPERSEDES the R11 "version_id/version_no remain GLOBALLY unique" decision | that decision assumed the serial per-advisor max-read always advanced; on the live machine it did not, and with version_id as the PRIMARY key the collision destroyed every advisor's version but the last. Advisor-in-the-key makes collisions impossible even under stale sequence reads |
| 2026-07-26 | R16: legacy sample versions v1–v24 / scans 001–005 keep their old-format ids; committed sample demo state untouched | readers resolve the id from the winning vertex (never parse it), so old and new formats coexist; verification runs in a temp copy (verify_per_advisor pattern), test-run CSV mutations reverted |
| 2026-07-26 | R16: supersede + _supersede_global_versions switched from local-store reads to get_commentary_versions; _next_scan_id from store reads to get_anomaly_scans | the store-only paths silently did nothing (supersede) or always returned scan001 (_next_scan_id) on tier 1 — the queries give both tiers identical behaviour, which C4 (verify supersede with the new key) requires |
| 2026-07-26 | R16: commentary_evaluation added to the migration clear list (spec C names 4 vertex types) | evaluation rows reference commentary_id/version_id of cleared rows — keeping them would leave exactly the dangling-reference class this round eliminates |
