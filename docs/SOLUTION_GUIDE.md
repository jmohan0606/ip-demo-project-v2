...
Written for a reader who is smart but new to this system. Worked examples in this
guide use real numbers from the shipped sample data set (`data/sample/`) — nothing is
invented. Where a value is an assumption or a placeholder, it says so.

> **Standing rule (Round 5): real data is the only demo path.** Every demo,
> screenshot and acceptance run uses `DATA_SET=real` against live TigerGraph.
> The sample set is a TEST ASSET: it feeds the automated verification scripts and
> this guide's worked examples, nothing more. `data/fixtures/` (generated by
> `scripts/make_ingestion_fixtures.py`) is the ingestion regression harness.

> **Standing rule (Round 11): every use case must be demonstrable on the SAMPLE
> data set.** When a feature is added, `scripts/generate_sample_data.py` must be
> extended so the sample exercises it — a feature only observable on real data
> cannot be verified here (real data is operator-local) and cannot be shown in a
> sample walk-through. The generator stays deterministic, sample reconciliation
> stays $0.00, and each crafted scenario carries a comment naming the use case it
> demonstrates. As of Round 11 the sample (Apr–Jul 2026) demonstrates: a 9G flip
> (INHERITANCE), a 9E flip (HOUSEHOLD), recurring AND non-recurring Annuities
> products, an in-scope chargeback (CLAWBACK) plus an out-of-scope reversal, a
> mixed one-time+recurring account (R9 exclusion), every anomaly rule, the
> 91/92/9L eligibility flip, and a MIX-clean May→Jun transition for every advisor
> (SMPL002 Jun→Jul is the one deliberately high-residual transition, crafted for
> the UNEXPLAINED_RESIDUAL rule).

Last updated: 2026-07-25 (Round 11 of the build, per `FIX_SPEC_R11.md`).
...
> **Real data only.** The demo and acceptance path is `DATA_SET=real` end to end —
> never present sample data as a demo. (Developers running automated tests use the
> sample set and `data/fixtures/`; that is a test loop, not a demo.) After the round-5
> ingestion fixes, complete `docs/ROUND5_ACCEPTANCE.md` on first live setup.
...
### 10.12 Streaming ingestion for multi-million-row loads (next round)

Round 5 made ingestion chunked (per-entity write batches with resumable
checkpoints, no full-file materialisation in memory) and honest (checkpoint only
